...
def main():
    ...
    #For Bayesian, let us consider 100 data points out of which initially 90 are unlabelled and 10 are labelled
    # yremoved = removelabels(90,Ytr[:100])
    ...
    for tval in range(5,50,5):
        #theta_rs = BayesianRandomSampling(UX, UY, LX, LY, T=int(tval / batchsize))
        theta_rs = BayesianRandomSampling(UX, UY, LX, LY, T=int(tval/batchsize))
        np.save('theta_rs.npy', theta_rs)
        balyte = np.argmax(BayesianPredict(svalue, Xte, theta_rs), axis=1)
        print 'accuracy_score_bayesian_rs', accuracy_score(Yte, balyte)
        RSaccuracy.append(accuracy_score(Yte, balyte))
        
        thetars = BayesianActiveLearning(UX,UY,LX,LY,T=int(tval/batchsize))
        #thetars = BayesianActiveLearning(UX, UY, LX, LY, T=int(tval))
        np.save('thetars.npy',thetars)
        bayesian_alyte = np.argmax(BayesianPredict(svalue,Xte,thetars),axis=1)
        print 'accuracy_score_bayesian_al',accuracy_score(Yte,bayesian_alyte)
        BALaccuracy.append(accuracy_score(Yte,bayesian_alyte))

        print 'finished tval = ',tval
    print RSaccuracy,BALaccuracy

    np.save('BALAccu100.npy', np.array(BALaccuracy))
    np.save('BRSAccu100.npy', np.array(RSaccuracy))
