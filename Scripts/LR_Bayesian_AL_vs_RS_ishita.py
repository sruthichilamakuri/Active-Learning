...
def main():
    ...
    # returnedobj = logregobj.fit(Xtr[:150], Ytr[:150])
    # # ytrlr = returnedobj.predict(Xtr)
    # # print accuracy_score(Ytr,ytrlr)
    # ytelr = returnedobj.predict(Xte)
    # print 'ytelr.shape', ytelr.shape
    # print 'Accuracy score for model trained on 100 data-cases: ', accuracy_score(Yte, ytelr)
    # print '***********************'
    ...
    # # ***********
    # np.save('UXtr100.npy', UX)
    # np.save('UYtr100.npy', UY)
    # np.save('LXtr100.npy', LX)
    # np.save('LYtr100.npy', LY)
    ...
    print BALaccuracy
    print querytime
    # np.save('BALAccu_S500.npy', np.array(BALaccuracy))
    np.save('batchtime.npy', np.array(querytime))
    # np.save('BRSAccu100_1.npy', np.array(RSaccuracy))
    fig, ax = plt.subplots()
    xaxis = np.arange(10, 100, 10)
    # ax.plot(xaxis, BALaccuracy, color='g', label='SAMPLE=500')
    ...
    plt.ylabel('Accuracy---->')
    plt.title('Effect of parameter sample size on Bayesian Active Learning')
    plt.grid(False)
    plt.show()
